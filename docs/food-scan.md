...
大图 → 标题/叙事 → 热量环 + 三大营养素 → **盘中食材** → 饮食记录 → **风味与口感** → 小贴士 → **实用建议/饮食文化** → 过敏原 → 追问芯片 → 对话区

> 注意：后端字段齐全但前端不渲染时，会感觉「比 Chance 内容少」。改 prompt 后务必同步 `FoodScanInsightSections`。详见 [近期改动与避坑-2026-08.md](./近期改动与避坑-2026-08.md)。
